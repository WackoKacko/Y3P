...
int T1 = 0; int T2 = 0;
byte stage = 0;
unsigned int preamble = 23999; //Pre-amble for security and to allow byte order to be determined on RX
...
if(stage==0||stage==1) {
  ...
  if(stage==2) { //if stage 2, provide "fine" feedback.
    ...
    if(L<220) data[2] = 50; //arbitrary output number 1
    if(M<220) data[3] = 60; //arbitrary output number 2
    if(R<220) data[4] = 70; //arbitrary output number 3
    HC12.write(data,sizeof(data));
